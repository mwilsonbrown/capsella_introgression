...
```{r}
# k=4 data prep
eu_k4_anc_info <- ancestry_location(eu_k4_anc, wgs)
...
```{r}
# new york map
ny_map + geom_point(data=subset(eu_k4, state == c("NY", "NJ")), aes(x=longitude, y=latitude, fill = anc_group), shape=21) +
...
I got rid of the posterior probability decoding, so no plots are here and at this time, I do nothing with the posterior data.
