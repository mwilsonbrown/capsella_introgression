...
```{r}
# tetsing with reproducible example
df <- as.matrix(data.frame(team=c(1:5, NA),
                 points=c(99, 90, 86, 88, NA, NA),
                 assists=c(33, NA, 31, 39, NA, NA),
                 rebounds=c(NA, 28, 24, 24, NA, NA)))

# trying with indexing
dropped_windows <- apply(df, 1, function(x) all(is.na(x)))
df2 <- df[!dropped_windows,]

# use dplyr to remove the last row


df %>% filter_at(vars(-team), all_vars(!is.na(.)))


df[rowSums(is.na(df)) != ncol(m), ]
```

```{r reformat eigeninfo for scaffold 10}
# apply reformatting function to each row of output matrix
pca10 <- apply(s10_500snp, 1, reform_map, dat = s10_500snp)
...
```{r}
pc_plot1 <- ggplot() +
  geom_point(data=subset(pca10_long, k3pop_sm != "Capsella bursa-pastoris" & window %in% as.character(c(1:names(pca10)[20]))), aes(x=PC1, y=PC2, color = k3pop_sm, shape= species), size = 3, alpha = 0.5) +
...
```{r}
sc10w19 <- ggplot() +
  geom_point(data=subset(pca10_long, window == "19" & k3pop_sm != "Capsella bursa-pastoris"), aes(x=PC1, y=PC2, color = k3pop_sm, shape= species), size = 3, alpha = 0.7) +
  scale_shape_manual(values = c('Capsella\ grandiflora'=15, 'Capsella\ orientalis'=18, 'Capsella\ bursa-pastoris'=16, 'Capsella\ rubella'=17)) + 
...
```{r}
ggsave(paste0(plot_dir,"lostruct_pca_scf10_window19.png"), sc10w19, width = 7, height = 5, units = "in")
```
