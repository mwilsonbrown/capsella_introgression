...
```{r}
pc_plot1 <- ggplot() +
  geom_point(data=lpca9_long, aes(x=PC1, y=PC2, color = k3pop_sm, shape= species), size = 3, alpha = 0.5) +
...
fill in NA for population
```{r}
# lpca10_long$k3population <- tidyr::replace_na(lpca10_long$k3population, "Other Sp.")
# lpca10_long$k3pop_sm <- tidyr::replace_na(lpca10_long$k3population, "Other Sp.")
...
```{r}
sc10w34 <- ggplot() +
  geom_point(data=subset(lpca10_long, window == 34), aes(x=PC1, y=PC2, color = k3pop_sm, shape= species), size = 3, alpha = 0.5) +
  theme_classic() + facet_wrap(~window)

sc10w34

ggsave("lostruct_pca_scf10_window34.png", sc10w34, width = 10, height = 6, units = "in")
```

```{r}
sc10w10 <- ggplot() +
  geom_point(data=subset(lpca10_long, window == 10), aes(x=PC1, y=PC2, color = k3pop_sm, shape= species), size = 3, alpha = 0.5) +
  theme_classic() + facet_wrap(~window)

sc10w10

ggsave("lostruct_pca_scf10_window10.png", sc10w10, width = 10, height = 6, units = "in")
```
