...
winds <- rep(1:5000, each = 100, length.out = nrow(genomat[["jlSCF_10"]])) #the end of rep is not important and should just be a sufficiently large number; each is the number passed to win in eigen_windows, and length.out should be the number of rows in the matrix

# combine with SNP location data
windows <- cbind(nyc_rubella_chr_snps[["jlSCF_10"]], as.character(winds))
```
