...
nyc_allc <- viterbi_columns_plot_allChr(vit_all, population = "NYC")

# why is the order wrong >:(
ggsave(paste0(temp_plotdir,"ny_viterbi_all4.png"),nyc_allc, units = "in", height = 12, width = 19)
```

```{r nyc_introgression_plot_allScaffolds}
neu_allc <- viterbi_columns_plot_allChr(vit_all, population = c("N_Europe"))

# why is the order wrong >:(
ggsave(paste0(temp_plotdir,"neu_viterbi_all4.png"),neu_allc, units = "in", height = 12, width = 19)
```

```{r nyc_introgression_plot_allScaffolds}
mena_allc <- viterbi_columns_plot_allChr(vit_all, population = c("MENA"))

# why is the order wrong >:(
ggsave(paste0(temp_plotdir,"mena_viterbi_all4.png"),mena_allc, units = "in", height = 12, width = 19)
```

```{r nyc_introgression_plot_allScaffolds}
easia_allc <- viterbi_columns_plot_allChr(vit_all, population = c("E_Asia"))

ggsave(paste0(temp_plotdir,"easia_viterbi_all4.png"),easia_allc, units = "in", height = 12, width = 19)
```
...
ggplot() + geom_segment(data = nyc_inter[,1:4], aes(colour= num, x=chrom, xend=chrom, y=start, yend=end), linewidth = 10) + scale_x_discrete(labels = paste0("chr ", 9:16)) + scale_color_scico(palette = 'acton', direction = -1) + theme_classic() + labs(title = "New York introgressed region frequency", colour = "individuals", x = NULL, y = "position")
...
row_sind <- seq(from = 1, to = nrow(sc9), by = 3000)
row_eind <- c(seq(from = 1, to = nrow(sc9), by = 3000)-1)[2:length(row_sind)]
winds <- data.frame(start = row_sind[-21], end = row_eind) #rows for extracting bp positions from names

# from the row names, get the staring and ending bp positions
bp_ends <- as.numeric(str_remove(rownames(sc9)[breaks$end], "jlSCF_9_"))
bp_beg <- as.numeric(str_remove(rownames(sc9)[breaks$start], "jlSCF_9_"))
...
ggsave("~/Desktop/scaffold9_window18.png", width = 5, height = 2)
```
...
ggplot() + 
  ...
  geom_rect(data = breaks,
            ...
            alpha = 0.5,
            fill = "#a9ba9d") +
  geom_hline(data = breaks, aes(yintercept = end, linetype = "dotted")) +
  geom_linerange(data = c_inter[[4]], linewidth = 5, aes(x=population, ymin=start, ymax=end, color = num)) +
  scale_color_scico(palette = 'lapaz', direction = -1) +
  new_scale_color() + 
  geom_linerange(data = c_inter[[5]], linewidth = 5, aes(x=population, ymin=start, ymax=end, color = num)) +
