...
t <- apply(sm_eu_inter["list"], 1, function(x){
# for each row
...
nel <- as.numeric(el)
# check if all 65 nyc inds are contained
sum(nel %in% ny_indx) == 65
})
...
ggplot() + geom_segment(data = sm_eu_inter[which(med_private),], aes(colour= num, x=chrom, xend=chrom, y=start, yend=end), linewidth = 10) + scale_color_continuous(high = "violetred4", low = "mistyrose") + ggtitle("introgressed regions private to Mediterranean") + theme_classic()
```
No regions found to be private to Northern European group, or NYC
...
# only in Mediterranean group
ggplot() + geom_segment(data = sm_eu_inter[which(cen_private),], aes(colour= num, x=chrom, xend=chrom, y=start, yend=end), linewidth = 10) + scale_color_continuous(high = "violetred4", low = "mistyrose") + ggtitle("introgressed regions private to Central Asia") + theme_classic()
```
...
I think from this, we can conclude that alleles shared across European populations are not due to shared introgression
#### Genic Content
