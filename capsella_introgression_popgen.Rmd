...
```
Last attempt but with widows in the backgroud
```{r}
# breakpoints for shading
row_sind <- seq(from = 1, to = nrow(sc9), by = 3000)
row_eind <- c(seq(from = 1, to = nrow(sc9), by = 3000)-1)[2:length(row_sind)]
breaks <- data.frame(start = row_sind[-21], end = row_eind) #rows for extracting bp positions from names
...
base <- ggplot() + geom_rect(data = breaks,
            aes(xmin = start,
                xmax = end,
                ymin = - Inf,
                ymax = Inf),
...
apply(c_inter, FUN=filter(chrom == "jlSCF_9"))
